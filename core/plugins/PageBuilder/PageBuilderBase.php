...
use plugins\PageBuilder\Traits\AssetsPathOverride;
...
{
    use LanguageFallbackForPageBuilder, RenderViewString, AssetsPathOverride;
    protected $args;
    ...
    public function __construct(array $args=[])
    {
        ...
        $this->base_preview_image_path = asset($this->setAssetsFilePath());
        try {
    ...
    public function get_settings()
    {
        ...
        if (!empty($widget_data)) {
            $addon_settings = $widget_data['addon_settings'];
            $formatted_data = WidgetBuilderSetup::format_widget_content($addon_settings, ['class' => false]);
            return json_decode($formatted_data, true);
        }
    ...
    public function admin_form_submit_button($text = null): string
    {
        ...
        return '<button class="cmnBtn btn_5 btn_bg_blue radius-5 btn-md widget_save_change_button">' . $button_text . '</button>';
    }
