                ...
                <div class="dashboard__inner__header">
                    <div class="dashboard__inner__header__flex">
                        <div class="dashboard__inner__header__left">
                            <h4 class="dashboard__inner__header__title">{{ __('All Notices') }}</h4>
                       </div>
                        <div class="dashboard__inner__header__right">
                            @can('notice-add')
                                <div class="btn-wrapper">
                                    <a href="{{ route('admin.add.notice.page') }}" class="cmnBtn btn_5 btn_bg_blue radius-5">{{ __('Add Notice') }}</a>
                                </div>
                            @endcan
                ...
                <div class="tableStyle_three mt-4">
                    <div class="table_wrapper custom_Table">
                        <div class="search_notice_result">
                            @include('backend.pages.notice.search-notice')
                        </div>
                    </div>
                </div>
            </div>
        ...
        (function(){
            ...
            $(document).ready(function(){
                ...
                function notices(page){
                    $.ajax({
                        url:"{{ route('admin.notice.paginate').'?page='}}" + page,
                        success:function(res){
                            $('.search_notice_result').html(res);
                        }
                    });
